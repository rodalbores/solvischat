...
const normalizeSafetyText = (text) => String(text || '')
  .normalize('NFD')
  .toLowerCase()
  .replace(/[’`]/g, "'")
  .replace(/[\u0300-\u036f]/g, '')
...
const findFaqResponse = (userInput) => {
  ...
  if (detectedLang === 'ko') {
    faqData = faqDataKo;
    defaultResponse = "좋은 질문이에요. 지금 질문하신 내용은 제가 정확히 확인할 정보가 부족할 수 있어서, Casa de la Familia (877) 611-2272로 직접 연락하시면 가장 정확하게 안내받으실 수 있어요. 원하시면 어떤 도움이 필요한지 조금 더 말씀해 주세요. 제가 같이 정리해 드릴게요.";
  } else if (detectedLang === 'es') {
    faqData = faqDataEs;
    defaultResponse = "Buena pregunta. Puede que no tenga el dato exacto para tu caso, así que lo mejor es llamar a Casa de la Familia al (877) 611-2272 para una respuesta más precisa. Si quieres, cuéntame un poco más de lo que necesitas y te ayudo a enfocarlo.";
  } else {
    faqData = faqDataEn;
    defaultResponse = "That is a really good question. I might not have the exact detail for your specific situation, so the best next step is to call Casa de la Familia at (877) 611-2272 for a precise answer. If you want, share a little more about what you are trying to figure out and I can help you narrow it down before you call.";
  }
...
const CHAT_STORAGE_KEY = 'stella.chat.messages.v1';
const USER_PROFILE_STORAGE_KEY = 'stella.chat.userProfile.v1';
...
const loadPersistedMessages = () => {
  ...
  try {
    ...
    return normalized.length ? normalized.slice(-MAX_STORED_MESSAGES) : [buildWelcomeMessage()];
  } catch (error) {
    console.warn('Could not load persisted chat history:', error);
    return [buildWelcomeMessage()];
  }
};

const cleanExtractedName = (rawName) => {
  if (!rawName) return '';
  ...
  const blocked = new Set([
    'name', 'my name', 'your name', 'unknown', 'none', 'n/a', 'test',
    'stella', 'here', 'fine', 'good', 'ok', 'okay', 'well', 'not',
    'looking', 'trying', 'interested', 'calling', 'writing', 'wondering',
    'having', 'feeling', 'going', 'doing', 'being', 'so', 'very', 'really',
    'just', 'also', 'new', 'a', 'the', 'in', 'on', 'at', 'to', 'for',
    'looking for', 'interested in', 'trying to', 'going to', 'not sure',
    'here to', 'here for', 'in need', 'in danger', 'in crisis',
    'a survivor', 'a victim', 'a patient', 'a client',
  ]);
  if (blocked.has(lowered)) return '';
  if (/^(looking|trying|interested|calling|writing|wondering|having|feeling|going|doing|being|so|very|really|just|also|not)\b/i.test(lowered)) return '';
...
const extractUserName = (text) => {
  ...
  const patterns = [
    /\bmy name is\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+){0,2})/,
    /\bmy name is\s+([a-z][a-z'-]{0,19})/i,
    /\bcall me\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)/,
    /\bcall me\s+([a-z][a-z'-]{0,19})/i,
    /\bi'?m\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\s*[.,!?]?\s*$/,
    /\bi am\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\s*[.,!?]?\s*$/,
    /\bme llamo\s+([A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19}(?:\s[A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19}){0,2})/i,
    /\bmi nombre es\s+([A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19}(?:\s[A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19}){0,2})/i,
    /\bsoy\s+([A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19}(?:\s[A-ZÁÉÍÓÚÑÜa-záéíóúñü][a-záéíóúñü'-]{0,19})?)\s*[.,!?]?\s*$/i,
    /(?:^|\s)나는\s+([^\s.,!?]{1,20})(?:야|이에요|입니다)/,
    /(?:^|\s)제 이름은\s+([^\s.,!?]{1,20})(?:이에요|입니다)/,
  ];
...
const loadPersistedUserName = () => {
  if (typeof window === 'undefined') return '';
  try {
    const raw = window.localStorage.getItem(USER_PROFILE_STORAGE_KEY);
    if (!raw) return '';
    const parsed = JSON.parse(raw);
    return cleanExtractedName(parsed?.name);
  } catch (error) {
    console.warn('Could not load persisted user profile:', error);
    return '';
  }
};
...
export default function StellaChatbot() {
  ...
  const [rememberedName, setRememberedName] = useState(loadPersistedUserName);
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({ name: rememberedName })
      );
    } catch (error) {
  ...
  const baseSystemPrompt = `You are Stella, a warm and caring support companion at Casa de la Familia. You talk like a real person - friendly, genuine, and conversational.
...
You're not a therapist - you're a friendly guide helping people find the right support at Casa de la Familia.`;
  ...
  const handleSend = async () => {
    ...
    const nameFromInput = extractUserName(userInput);
    const effectiveName = nameFromInput || rememberedName;
    if (nameFromInput && nameFromInput !== rememberedName) {
      setRememberedName(nameFromInput);
    }
    ...
    // Crisis detection guard (client-side): bypass API/FAQ and respond immediately.
    if (detectCrisis(userInput)) {
      await animateAssistantResponse(buildClientCrisisResponse(detectedLanguage));
      setIsLoading(false);
      return;
    }
    ...
    try {
      const stellaSystemPrompt = [
        baseSystemPrompt,
        effectiveName
          ? `USER MEMORY:\n- The user's name is ${effectiveName}.\n- If natural, occasionally address the user by this name.\n- Do not overuse the name in every message.`
          : '',
      ].filter(Boolean).join('\n\n');
      ...
      if (nameFromInput) {
        const nameGreetings = {
          ...
          ko: `안녕하세요 ${nameFromInput}님! 만나서 반갑습니다. 저는 Casa de la Familia의 Stella입니다. 오늘 어떻게 도와드릴까요?`,
        };
        await animateAssistantResponse(nameGreetings[detectedLanguage] || nameGreetings.en);
      } else {
